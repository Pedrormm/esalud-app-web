...
class CreateUsers extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->bigInteger('role_id')->unsigned()->comment('User role type');
            $table->string('address', 200)->nullable()->comment('User address');
            $table->string('city', 200)->nullable()->comment('User city');
            $table->bigInteger('country_id')->unsigned()->comment('Country id associated to the user');
            // $table->string('country', 100)->nullable()->comment('User country');
            $table->integer('zipcode')->nullable()->comment('User zipcode');
            // $table->bigInteger('phone_prefix_id')->unsigned()->comment('Telephone prefix id associated to the user');
            $table->bigInteger('phone_prefix_id')->unsigned()->index()->comment('Telephone prefix id associated to the user');
            $table->foreign('phone_prefix_id')->references('id')->on('phone_prefixes')->onDelete('cascade'); 
            ...
            $table->string('language', 10)->default('es')->comment('User current language');
            $table->softDeletes();
            $table->timestamps();
        });
