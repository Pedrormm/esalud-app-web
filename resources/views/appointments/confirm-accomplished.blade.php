                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <label id="sureAppointment" data-ap-id="{{ $appointment->id }}"
                                    ...
                                    ¿Seguro que desea {{ $accomplishedText }} la cita {{ $appointment->id }}?
                                </label>              
                            </div>
                        </div>
                    </div>
  ...
  <script>
    $('#saveModal').click(function() {
        ...
        saveModalActionAjax(PublicURL+"appointment/"+sureId+"/setAccomplished/"+{{ $accomplished }}, sureId, 'POST', 'json', function(res) {
            if(res.status == 0) {
